...
_FORECAST_CACHE_1 = {}
_CACHED_MODEL_1 = None

def demand_forecasting_main(file_names, product_name_input):
    global _CACHED_MODEL_1
    cache_key = str(product_name_input).strip().lower()
    if cache_key in _FORECAST_CACHE_1:
        return _FORECAST_CACHE_1[cache_key]
    ...
    model_path = 'shop1_model.h5'
    if _CACHED_MODEL_1 is not None:
        model = _CACHED_MODEL_1
    elif os.path.exists(model_path):
        model = create_lstm_model(sequence_length)
        model.load_weights(model_path)
        _CACHED_MODEL_1 = model
    else:
        print(f"[AI] Training new Shop 1 model...")
        model = create_lstm_model(sequence_length)
    ...
    for product_name in combine_df['Product Name'].unique():
        if product_name.strip().lower() == product_name_input.strip().lower():
            ...
            if predictions:
                ...
                _FORECAST_CACHE_1[cache_key] = res
                return res
            break
