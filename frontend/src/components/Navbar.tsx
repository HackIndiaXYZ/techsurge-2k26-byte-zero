  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-4">
        {role && (
          <>
            {role === 'owner' && <NavItem to="/owner" label="Commander" active={location.pathname === '/owner'} />}
            <NavItem to={`/store/${selectedStore || 'A'}`} label="My Store" active={location.pathname.startsWith('/store')} />
            <NavItem to="/orders" label="Orders" active={location.pathname === '/orders'} />

            {selectedStore && (
              <span className="px-3 py-1 rounded-full text-xs font-display tracking-wider bg-primary/10 text-primary border border-primary/20">
                Shop {selectedStore}
              </span>
            )}
            <span className="px-3 py-1 rounded-full text-xs font-body bg-secondary text-foreground/70 capitalize">
              {role}
            </span>
            ...
            <button onClick={reset} className="p-2 rounded-lg hover:bg-secondary transition-colors">
